...
loadCSV('交单时段.csv', function (submitCsv) {
  ...
  function renderSlotMonthlyForWeekday(weekday, containerId) {
    var range = getMonthRange();
    var startMonth = range.startMonth;
    var endMonth = range.endMonth;
  ...
  function renderAll() {
    ...
    // 默认不展示下方详细图，等待点击激活
    renderSlotMonthlyForWeekday('周一', 'container-time-submit-weekly-mon');
    renderSlotMonthlyForWeekday('周二', 'container-time-submit-weekly-tue');
    renderSlotMonthlyForWeekday('周三', 'container-time-submit-weekly-wed');
    renderSlotMonthlyForWeekday('周四', 'container-time-submit-weekly-thu');
    renderSlotMonthlyForWeekday('周五', 'container-time-submit-weekly-fri');
    renderSlotMonthlyForWeekday('周六', 'container-time-submit-weekly-sat');
    renderSlotMonthlyForWeekday('周日', 'container-time-submit-weekly-sun');
    setWeeklyVisible(null);
  }
  ...
  function exportExcelByMonthRange() {
    ...
    var ws1 = XLSX.utils.aoa_to_sheet([['月份', '时段', '该时段对应病例数量']].concat(s1Rows));
    var ws2 = XLSX.utils.aoa_to_sheet([['月份', '日期性质', '时段', '该时段对应病例数量']].concat(s2Rows));
    XLSX.utils.book_append_sheet(wb, ws1, '全时间段');
    XLSX.utils.book_append_sheet(wb, ws2, '按星期划分');
    XLSX.writeFile(wb, '交单数量_' + startMonth + '_到_' + endMonth + '.xlsx');
  }
