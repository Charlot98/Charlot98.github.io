(function () {
  ...
  loadCSV('csv/doctor_list.csv', function (listCsv) {
    ...
    loadCSV('csv/xray_project_by_doctor.csv', function (csv) {
      var table = parseCSV(csv);
      if (table.length < 2) {
        renderError('X 线项目统计数据为空，请先运行数据生成脚本。');
        return;
      }
      ...
      var projectIndex = header.indexOf('X线项目');
      ...
      if (
        ...
        renderError('X 线项目统计 CSV 表头不完整，请重新生成数据。');
        return;
      }
      ...
      document.getElementById('export-excel-btn').onclick = function () {
        ...
        var byProject = aggregateRows(selectedDoctors, range.startMonth, range.endMonth);
        var exportHeader = ['年月范围', 'X线项目', '医生编号', '医生', '医生等级', '报告数量', '审核数量', '总数量'];
        var exportRows = [];
        var rangeLabel = range.startMonth === range.endMonth
          ? range.startMonth
          : range.startMonth + ' 至 ' + range.endMonth;
        ...
        downloadExcel(
          exportHeader,
          exportRows,
          'X线项目数量_' + range.startMonth + '_' + range.endMonth + '.xlsx'
        );
      };
