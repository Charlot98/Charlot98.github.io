(function () {
  ...
  function createChartCard(project, rows, index, rangeLabel) {
    ...
    Highcharts.chart(chart.id, {
      ...
      series: [
        {
          name: project === 'MRI报告、审核总数' ? 'MRI审核总数' : '审核医生数量',
          data: auditData,
          color: '#f59e0b'
        },
        {
          name: project === 'MRI报告、审核总数' ? 'MRI报告总数' : '报告医生数量',
          data: reportData,
          color: '#4a90d9'
        }
      ]
  ...
  loadCSV('csv/doctor_list.csv', function (listCsv) {
    ...
    loadCSV('csv/mri_project_by_doctor.csv', function (csv) {
      var table = parseCSV(csv);
      if (table.length < 2) {
        renderError('MRI 项目统计数据为空，请先运行数据生成脚本。');
        return;
      }
      ...
      var projectIndex = header.indexOf('MRI项目');
      ...
      if (
        ...
        renderError('MRI 项目统计 CSV 表头不完整，请重新生成数据。');
        return;
      }
      ...
      document.getElementById('export-excel-btn').onclick = function () {
        ...
        var byProject = aggregateRows(selectedDoctors, range.startMonth, range.endMonth);
        var exportHeader = ['年月范围', 'MRI项目', '医生编号', '医生', '医生等级', '报告数量', '审核数量', '总数量'];
        var exportRows = [];
        var rangeLabel = range.startMonth === range.endMonth
          ? range.startMonth
          : range.startMonth + ' 至 ' + range.endMonth;
        ...
        downloadExcel(
          exportHeader,
          exportRows,
          'MRI项目数量_' + range.startMonth + '_' + range.endMonth + '.xlsx'
        );
      };
