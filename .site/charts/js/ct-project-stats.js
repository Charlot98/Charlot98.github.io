(function () {
  ...
  loadCSV('csv/doctor_list.csv', function (listCsv) {
    ...
    loadCSV('csv/ct_project_by_doctor.csv', function (csv) {
      var table = parseCSV(csv);
      if (table.length < 2) {
        renderError('CT / XA 统计数据为空，请先运行数据生成脚本。');
        return;
      }
      ...
      var projectIndex = header.indexOf('统计项目');
      ...
      document.getElementById('export-excel-btn').onclick = function () {
        ...
        downloadExcel(
          exportHeader,
          exportRows,
          'CT_XA统计_' + range.startMonth + '_' + range.endMonth + '.xlsx'
        );
      };
